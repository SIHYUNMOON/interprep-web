...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("ENV user len", (process.env.ADMIN_USERNAME ?? "adminInPrep2013").length);
    console.log("ENV pass len", (process.env.ADMIN_PASSWORD ?? "InterP!Web26#Ops@").length);
    console.log("REQ user len", username.length);
    console.log("REQ pass len", password.length);
